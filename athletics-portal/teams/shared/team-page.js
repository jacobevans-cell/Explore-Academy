import { TEAM_DATA, TEAM_PAGE_LINKS } from './team-data.js?v=20260920-practice-shift';
...
function teamIsInPracticeSeason(team,date){if(!teamIsInSeason(team,date))return false;const practiceEnd=parseSeasonDate(team.season?.practiceEnd,true);if(practiceEnd){const check=new Date(date);check.setHours(12,0,0,0);if(check>practiceEnd)return false;}return true;}
...
function outOfSeasonCard(team,day,date){
  ...
  const practiceEnded=!before&&team.season?.practiceEnd;
  const message=before?`Season begins ${team.season?.display||team.season?.start||''}`:practiceEnded?'Team practices have ended':'Season has ended';
